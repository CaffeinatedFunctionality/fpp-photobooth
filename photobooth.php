...
function SavePluginConfig() {
    ...
    $.post('/api/configfile/plugin.fpp-photobooth.json', configStr).done(function(data) {
        $.jGrowl('Video Capture Config Saved');
        SetRestartFlag(2);
        CheckRestartRebootFlags();
    }).fail(function() {
        alert('Error, could not save plugin.fpp-photobooth.json config file.');
    });
}

function LoadConfig() {
    $.ajax({
        url: 'api/configfile/plugin.fpp-photobooth.json',
        type: 'GET',
        async: false,
        dataType: 'json',
...
function LoadDevices() {
    $.get('plugin.php?plugin=fpp-photobooth&page=devices.php&nopage=1', function(data) {
        devices = data;
